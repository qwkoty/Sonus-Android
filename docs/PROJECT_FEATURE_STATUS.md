...
| 2026-07-11 | v1.26 | 紧急修复（先写方案 DEV_VIZ_FIX_v1.26.md）：v1.25 的 O2 在动画循环 ocean 位置分支与地形着色分支误用 `idx`（`idx` 仅为 buildBase 局部变量，动画循环索引是 `i`）→ 每帧 rAF 抛 ReferenceError 致动画循环停摆、地形模式卡死；两处改为 `terrainHN[i]`/`terrainRidge[i]`（与同分支 terrainBand[i] 索引语义一致，视觉/性能收益不变）。vite build 通过 | AI Assistant |
| 2026-07-11 | v1.27 | 修复 3D 360° 旋转失效（先写方案 DEV_VIZ_FIX_v1.27.md）：v1.25 的 O1 外层 key 从 `` `${vm}:${v3m}` `` 改为 `'3d':'2d'` 分组后，2D↔3D 切换时外层 div 完全 remount → Suspense fallback 时序 + 150ms vizFadeIn 动画导致 canvas 触控监听器绑定时机异常、Android WebView 触摸事件分发丢失 → 单指滑动无法旋转。回退外层 key 到 v1.24 策略 `` `${vm}:${v3m}` ``，保留内层 Suspense 无 key（封面由内部 useEffect 重载）。vite build 通过 | AI Assistant |
| 2026-07-11 | v1.28 | 移除「横向滑动切模式」手势（Player.jsx 背景层 `onTouchStart/onTouchEnd` 绑定 + `swipeRef/onVizTouchStart/onVizTouchEnd` 整段删除）；模式切换保留「视觉设置面板」按钮入口(`switchVm`)，`buzz` 触觉反馈保留。新增 DEV_REMOVE_SWIPE_RESTORE_360ROTATE.md。vite build + oxlint 0 error 通过（注：当时误判 360° 旋转已随滑动移除而"恢复"，实测仍失效，根因见 v1.29） | AI Assistant |
| 2026-07-12 | v1.29 | 真正修复 3D 360° 拖拽旋转失效（先写方案 DEV_FIX_360_ROTATE_GESTURE_v1.29.md）：根因并非旋转代码被删，而是**宿主 WebView 在原生层把手势劫持用于页面缩放**——`index.html` viewport 未禁缩放、`MainActivity` 未 `setSupportZoom(false)`（默认 true）导致 canvas 的 touch 手势被吞/延迟、整屏被缩放。修复：viewport 增加 `user-scalable=no,maximum-scale=1,minimum-scale=1`；`MainActivity` 增加 `setSupportZoom(false)`+`setBuiltInZoomControls(false)`；`Visualizer3D` 容器 div 兜底 `touch-action:none`。旋转手势逻辑本身正确、未改动；`autoRotate` 保持关闭（用户不要自动旋转）。vite build + oxlint 0 error 通过 | AI Assistant |
| 2026-07-12 | v1.30 | 修复 3D 360° 旋转/缩放在真机彻底不生效（先写方案 DEV_FIX_360_ROTATE_POINTER_v1.30.md）：v1.29 仅禁掉 WebView 原生页面缩放，但 canvas 的 touch 手势在 Android WebView 仍投递不可靠、收不到。根因：Three.js 运行时 `appendChild` 的 `<canvas>` 上挂的 touchstart/touchmove/touchend 移动端不可靠。改为 **Pointer Events + setPointerCapture**，监听绑定到 React 渲染的 container（zIndex:2 铺满全屏）而非动态 canvas，统一鼠标/触摸/笔；用 pointers Map 追踪多指——单指=旋转(偏航全向+俯仰±85°)、双指=纯缩放(0.4x–3.0x，移除原双指角度扭转使职责清晰)；保留 wheel 桌面缩放；`autoRotate` 保持关闭。vite build + oxlint 0 error 通过 | AI Assistant |
| 2026-07-12 | v1.31 | 修复 v1.0.144 仍可能失效：发现被遗漏的根因——`FloatingLyrics` 实为**全屏 `<canvas>` 装饰层**(inset:0,zIndex:1) 且未设 `pointer-events:none`（对比 `LyricScroll` 已设）；部分 Android WebView 下因 `vizFadeIn` 动画/stacking 细节盖在 3D 画布(container zIndex:2) 之上吞掉所有指针手势，致 `pointerdown` 根本到不了 container（故 v1.30 的 Pointer Events 也救不了）。修复：给 `FloatingLyrics` 根 canvas 加 `pointerEvents:'none'`（纯装饰层，与 LyricScroll 一致，视觉不变）；先写方案 DEV_FIX_FLOATING_LYRICS_POINTER_v1.31.md。vite build + oxlint 0 error 通过 | AI Assistant |
| 2026-07-12 | v1.32 | 修复 v1.0.145 QQ 音乐整类放不出（先写方案 DEV_FIX_QQ_PLAYBACK_BACKEND_v1.32.md）：根因——QQ 播放链路走前端原生直连(CookieReader.httpGet 请求 musicu.fcg 取 vkey) + 本地 NanoHTTPD 代理(localhost:port/?url=) 转发 CDN，真机上该原生/本地代理链路不稳定、易被风控致大量 QQ 歌曲取不到可播放 URL；而网易云早已统一走后端 /stream 代理一直正常。修复：qqSource 的 url/stream 改走后端 `apiUrl('/stream?platform=qq&id=...')`（与 neteaseSource 一致），由后端 getQQUrl 取 vkey 并代理音频流；登录态 cookie/uin 随 query 透传，VIP 歌曲仍可播。搜索/歌词/登录仍走前端原生(不变)。vite build + oxlint 0 error 通过 | AI Assistant |
| 2026-07-18 | v1.33 | 纠正 v1.32 QQ 播放修复的方向错误（先写方案 DEV_FIX_QQ_PLAYBACK_HYBRID_v1.33.md）：v1.32 把 QQ 的 url/stream 整条改为后端 /stream 代理，但实测 QQ 对服务端 IP 的 vkey 接口有风控(500003)，后端在云服务器上取不到 purl，导致 v1.0.146 的 QQ 确定性更糟（且删掉了原生兜底）。修正：恢复「原生设备链路优先(urlAPK，走设备真实 IP+设备 Cookie，绕开服务端风控) + 后端 /stream 兜底」的混合方案(qqUrlHybrid)；原生/代理/WebView 等基础设施经核查均健康且自 v1.27 未变。后端 getQQUrl 静态 guid 改为动态随机提升兜底成功率。搜索/歌词/登录/网易云不变。vite build + oxlint 0 error 通过 | AI Assistant |
