...
| 2026-07-11 | v1.28 | 移除「横向滑动切模式」手势（Player.jsx 背景层 `onTouchStart/onTouchEnd` 绑定 + `swipeRef/onVizTouchStart/onVizTouchEnd` 整段删除）；该手势此前与 3D 可视化 360° 拖拽旋转抢占同一横向手势，移除后 360° 旋转恢复（旋转代码本就在 Visualizer3D.jsx，并未被真正删除，无需新增代码）。模式切换保留「视觉设置面板」按钮入口(`switchVm`)，`buzz` 触觉反馈保留。新增 DEV_REMOVE_SWIPE_RESTORE_360ROTATE.md。vite build + oxlint 0 error 通过 | AI Assistant |
