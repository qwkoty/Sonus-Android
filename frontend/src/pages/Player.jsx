...
export default function Player({ onProfile }) {
  ...
  return (
    <div style={{ height: '100%', position: 'relative', overflow: 'hidden', background: '#000' }}>
      ...
      {/* 未展开：左下角迷你卡片 */}
      {!expanded && (
        <div className="glass-panel-strong" onClick={() => setExpanded(true)} style={{ position: 'absolute', bottom: 'calc(14px + var(--safe-bottom))', left: 'calc(16px + var(--safe-left))', zIndex: 100, borderRadius: 18, padding: '6px 8px 6px 6px', display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, cursor: 'pointer' }}>
      ...
      {/* 展开态：底部中央完整控制面板（只保留核心控制） */}
      {expanded && (
        <div className="glass-panel-strong" style={{ position: 'absolute', bottom: 'calc(14px + var(--safe-bottom))', left: '50%', transform: 'translateX(-50%)', width: 'min(520px, calc(100% - 32px))', zIndex: 100, borderRadius: 22, padding: '14px 18px' }}>
          {/* 进度条 */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            <RoundBtn onClick={toggleMode} title="播放模式" style={{ color: mc }}>{mi}</RoundBtn>
            ...
            <RoundBtn onClick={() => setQo(true)} title="播放队列"><ListMusic size={15} /></RoundBtn>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12 }}>
            ...
            <button onClick={() => setExpanded(false)} className="glass-button" style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'var(--text-secondary)' }} title="收起"><Maximize2 size={13} /></button>
          </div>
        </div>
      )}
