...
const VIZ_MODES = [{ key:'ring',label:'环',icon:'◯'},{ key:'wave',label:'波',icon:'〜'},{ key:'3d',label:'3D',icon:'◆'},{ key:'pulse',label:'脉冲',icon:'✦' }];
...
function Sheet({ open, onClose, title, children, h='80vh' }) {
  if(!open) return null;
  return (
    <div style={{position:'absolute',inset:0,zIndex:200,background:'rgba(0,0,0,0.25)',backdropFilter:'blur(8px)',WebkitBackdropFilter:'blur(8px)',animation:'fadeIn .2s ease both',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={onClose}>
      <div className="glass-panel-strong animate-slideUp" onClick={e=>e.stopPropagation()} style={{width:'70%',maxWidth:520,maxHeight:h,borderRadius:'18px',display:'flex',flexDirection:'column',overflow:'hidden'}}>
...
export default function Player({ onProfile }) {
  ...
  const [sq, setSq] = useState(false); const [qo, setQo] = useState(false);
  ...
  return (
    <div style={{height:'100%',position:'relative',overflow:'hidden',background:'#000',display:'flex',flexDirection:'column'}}>
      {/* ===== 可视化区（撑满） ===== */}
      <div style={{flex:1,position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'absolute',top:0,left:0,right:0,padding:'calc(10px + var(--safe-top)) 14px 8px',display:'flex',alignItems:'center',justifyContent:'space-between',zIndex:50,gap:8}}>
          ...
          <div style={{fontSize:14,fontWeight:700,flex:1,textAlign:'center',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:'#fff',textShadow:'0 2px 8px rgba(0,0,0,0.8)'}}>
        ...
        {/* 当前歌词（中部偏上） */}
        <div style={{position:'absolute',top:'38%',left:0,right:0,display:'flex',justifyContent:'center',padding:'0 24px',zIndex:10,pointerEvents:'none'}}>
        ...
        {/* 错误提示（玻璃） */}
        {error && <div className="glass-panel" style={{position:'absolute',top:'calc(52px + var(--safe-top))',left:'50%',transform:'translateX(-50%)',zIndex:300,padding:'8px 16px',borderRadius:12,fontSize:12,color:'#FCA5A5',background:'rgba(180,40,40,0.2)',borderColor:'rgba(248,113,113,0.3)'}}>{error}</div>}
      </div>
      ...
      <div className="glass-panel-strong" style={{
        ...
        borderRadius:0, borderTop:'1px solid rgba(255,255,255,0.06)',
      }}>
        {/* 进度条（占大头） */}
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
          <button onClick={toggleMode} className="glass-button" style={{width:32,height:32,borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center',color:mc}} title="播放模式">{mi}</button>
          ...
          <button onClick={togglePlay} className="glass-button-accent" style={{width:48,height:48,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',background:ac,boxShadow:`0 0 20px ${ac}55`}}>
          ...
          <button onClick={()=>setQo(true)} className="glass-button" style={{width:32,height:32,borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center'}} title="播放队列"><ListMusic size={15} color="var(--text-secondary)"/></button>
        </div>
